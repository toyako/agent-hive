...
export class ClaudeAdapter implements AgentAdapter {
  ...
  capabilities = ["review", "architecture", "coding", "planning"];

  async detect(): Promise<boolean> {
    // Check CLI or API key availability
    try {
      const result = await this.exec(CLAUDE_BIN, ["--version"], {}, 5000);
      if (result.exitCode === 0 && result.stdout.includes("Claude")) return true;
    } catch {}
  ...
  async execute(task: Task, instruction?: string): Promise<AgentResult> {
    ...
    try {
      ...
      const res = await client.chat.completions.create({
        model: cfg.model,
        messages: [
          { role: "system", content: "You are a skilled software engineer. Execute the task precisely. Output clean, working code." },
      ...
      // Handle Mimo's reasoning_content field
...
请审核以上结果并返回 JSON。`;
  ...
  async converse(messages: MessageEnvelope[]): Promise<MessageEnvelope> {
    ...
    const chatMessages = messages.map(m => ({
      role: (m.from === "claude" ? "assistant" : "user") as "assistant" | "user",
